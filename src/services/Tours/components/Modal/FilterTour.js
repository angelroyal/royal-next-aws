...
import { getFilters, filterStart } from "../../../config/Tour/filtersTours";
...
export default function FilterTour(props) {
    //Props
    const { auxTourData, setAuxTourData, tourData, setChangeTours, orderTour } = props;
    ...
    const handleApplyClick = () => {
        ...
        if (range[0] !== '' && range[1] !== '' && parseFloat(range[0]) > parseFloat(range[1])) {
            setRange([range[1], range[0]]);
        }

        if (range[0] !== '' || range[1] !== '') {

            let min = range[0] === '' ? Number.MIN_SAFE_INTEGER : parseFloat(range[0]);
            let max = range[1] === '' ? Number.MAX_SAFE_INTEGER : parseFloat(range[1]);
    ...
    const handleCheckbox = (event, type) => {
        ...
        const newFilters = type === 'checkbox' ? { ...filters } : { ...filtersStarts };
        // console.log(type);
        ...
        const values = newItems.filter(item => item.checked !== false).map(item => item.value);
    ...
    const handleClickReset = () => {
        const newFilterStart = { ...filterStart, stars: { items: [{ ...filterStart.stars.items[0], checked: true }] } };
        ...
        <>
            <div className='cont-filter-tour'>
                ...
                <div className='d-flex justify-content-between cont-btn-reset-result'>
                    <h7 className="text-black-filter d-flex align-items-center">{languageData.titlesFilterTour.filterResults}</h7>
                    <Button
                        className='text-blue-filter-tour'
                ...
                <Accordion defaultExpanded className='cont-accordion-filter-tour'>
                    ...
                    >
                        <h6 className="title-filters-accordion-tour">{languageData.titlesFilterTour.priceRange}</h6>
                    ...
                    <AccordionDetails>
                        <div className='d-flex justify-content-between'>
                            <div className='d-flex flex-column gap-2'>
                                <span className='text-grey-filter-tour-s'>{languageData.titlesFilterTour.minimum}</span>
                                <span className='text-grey-filter-tour-s'>${rangeValue[0].toLocaleString()}</span>
                            </div>

                            <div className='d-flex flex-column gap-2'>
                                <span className='text-grey-filter-tour-s'>{languageData.titlesFilterTour.maximum}</span>
                                <span className='text-grey-filter-tour-s'>${rangeValue[1].toLocaleString()}</span>
                            </div>
                        </div>
                        ...
                        <div className='d-flex justify-content-between'>
                            <div className='min-and-max-cont'>
                                <span className='text-grey-filter-tour-s'>{languageData.SearchBox.tabTour.from}</span>
                                <input
                                    type='text'
                            ...
                            <div className='min-and-max-cont'>
                                <span className='text-grey-filter-tour-s'>{languageData.titlesFilterTour.to}</span>
                                <input
                                    type='text'
                                    className='cont-number-range-price'
                    ...
                    <AccordionActions>
                        <Button className='btn-apply-range-price'
                ...
                <Accordion defaultExpanded className='cont-accordion-filter-tour'>
                    <AccordionSummary
                        ...
                        className='accordion-summary-filter-tour'
                    >
                        <h6 className="title-filters-accordion-tour">{languageData.titlesFilterHotel.Star}</h6>

                    </AccordionSummary>
                    <AccordionDetails>

                        <div className='d-flex gap-4'>
                            <div className={auxStart > 2 ? 'cont-stars-filter-tour-two' : 'cont-stars-filter-tour'}>

                                {filtersStarts.stars.items.length > 0 &&
                                    filtersStarts.stars.items.map((values, key) => (
                                        <div key={key} className={(key === 0 || key === 5) ? 'd-flex gap-2' : 'd-flex gap-1 mb-1'}>
                                            <input
                                                id='all-stars-a'
                                                type='checkbox'
                                                ...
                                                className="filters-tour-checkbox"
                                            />
                                            <label for="all-stars-a" className={key > 0 ? 'star-center-gap cont-checkbox-cursor' : 'text-black-filter-s cont-checkbox-cursor'}>
                                                {
                                                    key > 0 ? (
                                                        [...Array(values.value)].map((value, index) => <IconStar key={index} />)
                                                    )
                                                        : <span>{languageData.optionsFilterHotel.allStart}</span>
                                                }
                                            </label>
                                            {
                                                auxStart < 2 &&
